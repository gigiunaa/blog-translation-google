...
def translate_html():
    try:
        ...
        body_attrs = ' '.join([f'{k}=\"{v}\"' if isinstance(v, str) else f'{k}=\"{" ".join(v)}\"' for k, v in (body_tag.attrs.items() if body_tag else [])])
...
def health():
    return jsonify({'status': 'healthy', 'service': 'HTML Translator'})
